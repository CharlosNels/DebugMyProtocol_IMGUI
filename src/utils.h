...
#define SPDLOG_LOGGER_CALL_(level, ...) spdlog::log(spdlog::source_loc{__FILE__, __LINE__, SPDLOG_FUNCTION}, level, __VA_ARGS__)
...
template<class T>
T myFromLittleEndianByteSwap(const void *src)
{
...
template<class T>
void myToLittleEndianByteSwap(T val, void *dest)
{
    const size_t size = sizeof(T);
    char *src = (char *)&val;
...
template<class T>
void myToBigEndianByteSwap(T val, void *dest)
{
    char *src = (char *)&val;
    char *dest_ = (char *)dest;
    for(int i = 0;i < sizeof(T); ++i)
...
template<class T>
void myToLittleEndian(T val, void *dest)
{
    memcpy(dest, &val, sizeof(T));
}

template<class T>
T myFromBigEndian(const void *src)
{
    char const *src_ = (const char *)src;
    const size_t size = sizeof(T);
    T ret{};
...
template<class T>
void myToBigEndian(T val, void *dest)
{
    const size_t size = sizeof(T);
    char *src = (char *)&val;
    char *dest_ = (char *)dest;
...
std::vector<K> getKeysOfMap(const std::unordered_map<K, V> &map)
{
    std::vector<K> keys;
